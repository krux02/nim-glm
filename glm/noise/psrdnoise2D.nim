...
proc rgrad2(p: Vec2; rot: float): Vec2 =
  when false:
...
proc psrdnoise(pos: Vec2; per: Vec2; rot: float): Vec3 =
  ...
  var  uv: Vec2 = vec2(pos.x + pos.y*0.5, pos.y);
  
  var  i0: Vec2 = floor(uv);
  var  f0: Vec2 = fract(uv);
  # Traversal order
  var  i1: Vec2 = if f0.x > f0.y: vec2(1.0, 0.0) else: vec2(0.0, 1.0);

  # Unskewed grid points in (x,y) space
  var  p0: Vec2 = vec2(i0.x - i0.y * 0.5, i0.y);
  var  p1: Vec2 = vec2(p0.x + i1.x - i1.y * 0.5, p0.y + i1.y);
  var  p2: Vec2 = vec2(p0.x + 0.5, p0.y + 1.0);

  # Integer grid point indices in (u,v) space
  i1 = i0 + i1;
  var  i2: Vec2 = i0 + vec2(1.0, 1.0);
  ...
  var  xw: Vec3 = mod(vec3(p0.x, p1.x, p2.x), per.x);
  var  yw: Vec3 = mod(vec3(p0.y, p1.y, p2.y), per.y);
  var  iuw: Vec3 = xw + 0.5 * yw;
  var  ivw: Vec3 = yw;
  
  # Create gradients from indices
  var  g0: Vec2 = rgrad2(vec2(iuw.x, ivw.x), rot);
  var  g1: Vec2 = rgrad2(vec2(iuw.y, ivw.y), rot);
  var  g2: Vec2 = rgrad2(vec2(iuw.z, ivw.z), rot);

  # Gradients dot vectors to corresponding corners
  # (The derivatives of this are simply the gradients)
  var  w: Vec3 = vec3(dot(g0, d0), dot(g1, d1), dot(g2, d2));
  
  # Radial weights from corners
  # 0.8 is the square of 2/sqrt(5), the distance from
  # a grid point to the nearest simplex boundary
  var  t: Vec3 = 0.8 - vec3(dot(d0, d0), dot(d1, d1), dot(d2, d2));

  # Partial derivatives for analytical gradient computation
  var  dtdx: Vec3 = -2.0 * vec3(d0.x, d1.x, d2.x);
  var  dtdy: Vec3 = -2.0 * vec3(d0.y, d1.y, d2.y);
  ...
  var  dt0: Vec2 = vec2(dtdx.x, dtdy.x) * 4.0 * t3.x;
  var  dn0: Vec2 = t4.x * g0 + dt0 * w.x;
  var  dt1: Vec2 = vec2(dtdx.y, dtdy.y) * 4.0 * t3.y;
  var  dn1: Vec2 = t4.y * g1 + dt1 * w.y;
  var  dt2: Vec2 = vec2(dtdx.z, dtdy.z) * 4.0 * t3.z;
  var  dn2: Vec2 = t4.z * g2 + dt2 * w.z;
...
proc srdnoise(pos: Vec2; rot: float): float =
  ...
  var x: Vec3 = vec3(p0.x, p1.x, p2.x);
  var y: Vec3 = vec3(p0.y, p1.y, p2.y);
  var iuw: Vec3 = x + 0.5 * y;
  var ivw: Vec3 = y;
